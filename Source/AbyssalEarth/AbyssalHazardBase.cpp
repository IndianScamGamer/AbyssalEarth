...
void AAbyssalHazardBase::BeginPlay()
{
    ...
    if (bStartActive)
    {
        const float OffsetSeconds = bRandomizeInitialPhaseOffset
            ? FMath::FRandRange(0.0f, IdleDuration)
            : InitialPhaseOffset * IdleDuration;
...
{
    if (bHazardActive == bNewActive) { return; }
...
void AAbyssalHazardBase::EnterPhase(EHazardPhase NewPhase)
{
    ...
    if (Duration > 0.0f)
    {
        GetWorldTimerManager().SetTimer(PhaseTimerHandle, this, &AAbyssalHazardBase::AdvancePhase, Duration, false);
    }
    else
    {
...
{
    if (DamageMode == EHazardDamageMode::None) { return; }
    GetWorldTimerManager().SetTimer(DamageTimerHandle, this, &AAbyssalHazardBase::TickDamage, DamageTickInterval, true, 0.0f);
}
...
{
    if (!bHazardActive || CurrentPhase != EHazardPhase::Active) { return; }
    ...
    {
        UGameplayStatics::ApplyRadialDamage(World, DeltaDamage, GetActorLocation(), DamageRadius,
            HazardDamageType, TArray<AActor*>{ this }, this, GetInstigatorController(), false);
    }
    else if (DamageMode == EHazardDamageMode::Overlap)
    {
        for (AActor* Target : OverlappingActors)
        {
            if (IsValid(Target))
            {
                UGameplayStatics::ApplyDamage(Target, DeltaDamage, GetInstigatorController(), this, HazardDamageType);
            }
        }
    }
}
...
{
    if (OtherActor && OtherActor != this) { OverlappingActors.Add(OtherActor); }
}

void AAbyssalHazardBase::HandleOverlapEnd(UPrimitiveComponent* OverlappedComp, AActor* OtherActor,
    UPrimitiveComponent* OtherComp, int32 OtherBodyIndex)
{
    if (OtherActor) { OverlappingActors.Remove(OtherActor); }
}
